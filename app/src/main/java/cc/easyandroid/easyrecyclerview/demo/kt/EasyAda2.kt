package com.house101.app1.kt.adapter
...
abstract class EasyAda2< T : IFHolder_Kt, VH : RecyclerView.ViewHolder> @JvmOverloads constructor(
            diffCallback: DiffUtil.ItemCallback<T>,
            mainDispatcher: CoroutineDispatcher = Dispatchers.Main,
            workerDispatcher: CoroutineDispatcher = Dispatchers.Default
    ) : EasyFlexibleAdapter<T>() {
        ...
        init {
            ...
            fun considerAllowingStateRestoration() {
                if (stateRestorationPolicy == PREVENT && !userSetRestorationPolicy) {
                    this@EasyAda2.stateRestorationPolicy = ALLOW
                }
            }
            ...
            addLoadStateListener(object : Function1<CombinedLoadStates, Unit> {
                ...
                override fun invoke(loadStates: CombinedLoadStates) {
                    println("cgpEasyAda1 $loadStates.source")
                    if (ignoreNextEvent) {
                        ignoreNextEvent = false
                    } else if (loadStates.source.refresh is NotLoading) {
                        println("cgpEasyAda2 $loadStates.source")
                        considerAllowingStateRestoration()
         ...
         */
       override  fun getItem(@IntRange(from = 0) position: Int) = differ.getItem(position)
